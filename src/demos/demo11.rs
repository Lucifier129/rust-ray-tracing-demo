...
use crate::camera::PositionalCamera;
...
static FILENAME: &'static str = "dist/11.ppm";
...
pub fn run() -> io::Result<()> {
  ...
  let camera = PositionalCamera::new(
    Vec3(-2.0, 2.0, 1.0),
    Vec3(0.0, 0.0, -1.0),
    Vec3(0.0, 1.0, 0.0),
    20.0,
  ...
  let mut sphere_4 = Sphere::new(Vec3(-1.0, 0.0, -1.0), -0.45);

  let material_0 = Lambertian::new(Vec3(0.1, 0.2, 0.5));
  let material_1 = Lambertian::new(Vec3(0.8, 0.8, 0.0));
  let material_2 = Metal::new(Vec3(0.8, 0.6, 0.2)).set_fuzz(0.3);
  ...
  sphere_4.set_material(Box::new(material_4));
